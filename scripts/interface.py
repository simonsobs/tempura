#
# This code automatically generate python interface files containing docstring
...
def func_rm_args_opt(Func,pset,ispops=False):
...
def func_rm_args(args,pstring):
    # remove args which contains pstring
    new_args = args.copy()
    for p in args:
        if pstring in p:  new_args.remove(p)
    return new_args


def func_ch_args(args,pset):
    # change args contained in pset
...
parser = argparse.ArgumentParser(description='scan f90 file and create python interface files for calling f2py functions')
parser.add_argument('-moddir','--fdir',type=str,nargs='+',default='')
