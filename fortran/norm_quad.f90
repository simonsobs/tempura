...
!*
  ...
  case('lens')
    call get_lfac(lmax,lfac,lk2)
    call Kernels_lens(rL,W(1,:),Wg(1,:),SG(1,:,:),'S0')
    call Kernels_lens(rL,W(2,:),Wg(2,:),SG(2,:,:),'G0')
  end select
...
!*
  ...
  if (QDO(4).and.QDO(5))  call quad_tbeb(est,lmax,rlmin,rlmax,fC(EE,:),fC(BB,:),fC(TE,:),OC(TT,:),OC(EE,:),OC(BB,:),OC(TE,:),Ilg(4,:),Ilc(4,:),lfac)
